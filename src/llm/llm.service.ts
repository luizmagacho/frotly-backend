...
const INTENT_SYSTEM = `You are an AI assistant for a Brazilian Fleet Management software (SaaS). Parse the user message and reply with ONLY valid JSON (no markdown) matching this shape:
{"intent":"CREATE_RENTAL"|"MAINTENANCE_REPORT"|"UNKNOWN","confidence":0-1,"needs_clarification":boolean,"clarifying_questions":string[],"entities":{"vehicleHint"?:string,"driverHint"?:string,"licensePlate"?:string,"amount"?:number,"duration"?:string}}
Rules: Extract entities accurately. If the user wants to rent a car, "intent" is "CREATE_RENTAL". If they report a breakdown or service, "intent" is "MAINTENANCE_REPORT".`;

const CASHFLOW_SYSTEM = `You are a financial AI agent for a Brazilian Fleet Management platform.
Analyze the provided financial transaction or invoice and reply with ONLY valid JSON (no markdown) in this exact format:
{"category":"aluguel_recebido"|"manutencao_paga"|"imposto"|"multa"|"outros","customerHint":string|null,"vehicleHint":string|null,"isRevenue":boolean,"confidence":0-1,"notes":string}
Rules:
- Identify if it's income from rental (aluguel_recebido, isRevenue: true) or expense like mechanic/parts (manutencao_paga, isRevenue: false).
- customerHint = name of the driver or supplier.
- vehicleHint = license plate or model if mentioned.
